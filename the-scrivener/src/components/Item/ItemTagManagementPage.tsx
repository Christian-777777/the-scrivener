...
export function ItemTagManagementPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '32px 48px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: '16px', borderBottom: '2px solid var(--color-accent)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <h2 style={{ fontSize: '24px', color: GOLD, margin: 0, letterSpacing: '5px' }}>物品分类标签管理</h2>
          </div>
          <span style={{ fontSize: '14px', color: 'var(--color-text-muted)' }}>
            {worldTags.length} 个标签 · {worldItems.length} 件物品
          </span>
        </div>
      </div>
      ...
      <div style={{ padding: '14px 48px 0', flexShrink: 0, fontSize: '14px', color: 'var(--color-text-light)', lineHeight: '1.8' }}>
        修改标签颜色或名称将<b style={{ color: GOLD }}>自动同步</b>到所有带此标签的物品。
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '20px 48px 32px 48px', marginRight: '8px' }}>
        {worldTags.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--color-text-muted)', fontSize: '15px', lineHeight: '2' }}>
            暂无分类标签<br />
            <span style={{ fontSize: '13px' }}>点击上方「创建新标签」或在物品编辑器中添加标签后，会出现在这里。</span>
          </div>
        ) : (
              ...
              return (
                <div key={tag.id} style={{
                  ...
                  onMouseEnter={(e) => {
                    ...
                    e.currentTarget.style.boxShadow = `0 2px 12px ${tag.color}18`
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                    ...
                    <button className="btn btn-danger" style={{ fontSize: '13px', padding: '7px 18px' }}
                      onClick={() => { if (confirm(`删除标签「${tag.name}」？此操作不删除物品。`)) deleteItemTag(tag.id) }}>删除</button>
                  </div>
                </div>
              )
            })}
      ...
      {/* 编辑弹窗（createPortal，布局零抖动） */}
      {editingTag && (
        <EditTagModal tag={editingTag} usage={tagUsage[editingTag.name] || 0}
          onSave={(name, color) => { updateItemTag(editingTag.id, { name, color }); setEditingTag(null) }}
      ...
      {showCreate && (
        <CreateTagModal
          onSave={(name, color) => { addItemTag({ name, color, worldId: currentWorldId! }); setShowCreate(false) }}
...
function EditTagModal({ tag, usage, onSave, onClose }: {
  tag: ItemTag; usage: number; onSave: (name: string, color: string) => void; onClose: () => void
}) {
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '480px', width: '92%' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            border: `3px solid ${darken(color, 0.25)}`, flexShrink: 0,
        ...
        <div className="form-actions">
          <button className="btn" onClick={onClose} style={{ fontSize: '13px', padding: '7px 18px' }}>取消</button>
          <button className="btn btn-primary" disabled={!name.trim()}
            onClick={() => onSave(name.trim(), color)} style={{ fontSize: '13px', padding: '7px 18px' }}>
            保存
          </button>
        </div>
      </div>
...
function CreateTagModal({ onSave, onClose }: {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '480px', width: '92%' }}>
        ...
        <div className="form-group">
          <label>标签名称 *</label>
          <input className="input" value={name} onChange={(e) => setName(e.target.value)}
            autoFocus placeholder="输入标签名称..." style={{ fontSize: '15px', padding: '10px 14px' }}
            onKeyDown={(e) => { if (e.key === 'Enter' && name.trim()) onSave(name.trim(), color) }} />
        </div>
