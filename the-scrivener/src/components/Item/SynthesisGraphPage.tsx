// ========================================
// 合成图谱 — 左侧搜索+物品列表 | 书脊 | 右侧合成画布
// 菱形节点 · SVG连线 · 拖入添加 · 右键编辑关系
// ========================================

import { useState, useCallback, useRef, useMemo } from 'react'
import { createPortal } from 'react-dom'
import { useAppStore } from '@/store/useAppStore'
import type { Item, SynthesisGraph } from '@/types'
...
// ═══ 工具函数 ═══
function primaryColor(item: Item): string { return item.tags[0]?.color || GOLD }
...
// ═══ 主组件 ═══
export function SynthesisGraphPage() {
  const currentWorldId = useAppStore((s) => s.currentWorldId)
  const items = useAppStore((s) => s.items)
  const synthesisGraphs = useAppStore((s) => s.synthesisGraphs)
  const currentGraphId = useAppStore((s) => s.currentSynthesisGraphId)
  const addSynthesisGraph = useAppStore((s) => s.addSynthesisGraph)
  const updateSynthesisGraph = useAppStore((s) => s.updateSynthesisGraph)
  const deleteSynthesisGraph = useAppStore((s) => s.deleteSynthesisGraph)
  const setCurrentSynthesisGraph = useAppStore((s) => s.setCurrentSynthesisGraph)
  const updateItem = useAppStore((s) => s.updateItem)
  const goBack = useAppStore((s) => s.goBack)

  const worldItems = useMemo(() => items.filter((i) => i.worldId === currentWorldId), [items, currentWorldId])
  const worldGraphs = useMemo(() => synthesisGraphs.filter((g) => g.worldId === currentWorldId), [synthesisGraphs, currentWorldId])
  const graph = worldGraphs.find((g) => g.id === currentGraphId) || null

  // ═══ 左侧搜索 ═══
  const [search, setSearch] = useState('')
  const leftItems = useMemo(() => {
    let list = worldItems.filter((i) => !!i.name)
    if (search.trim()) {
  ...
  const graphItemIds = graph?.itemIds || []
  const sortedIds = [...graphItemIds].sort().join(',')
  if (sortedIds !== lastLayoutRef.current && graphItemIds.length > 0) {
    lastLayoutRef.current = sortedIds
    const exist: Record<string, { x: number; y: number }> = { ...nodePositions }
    const cx = 380, cy = 300, rad = Math.min(240, graphItemIds.length * 50)
    const layout = layoutCircular(graphItemIds.length, cx, cy, rad)
    graphItemIds.forEach((id, i) => { if (!exist[id]) exist[id] = layout[i] })
    for (const k of Object.keys(exist)) { if (!graphItemIds.includes(k)) delete exist[k] }
  ...
  const [editingRelation, setEditingRelation] = useState<SynthesisGraph['relations'][number] | null>(null)
  ...
  // ═══ 拖拽即时 DOM 更新（零延迟） ═══
  const applyDragToDOM = useCallback((itemId: string) => {
    const pos = dragPosRef.current
    const nodeEl = document.querySelector(`[data-item-id="${itemId}"]`) as HTMLDivElement | null
    if (nodeEl) {
      nodeEl.style.left = `${pos.x}px`
      nodeEl.style.top = `${pos.y}px`
      const labelEl = nodeEl.parentElement?.children[1] as HTMLDivElement | undefined
      if (labelEl) {
        labelEl.style.left = `${pos.x}px`
        labelEl.style.top = `${pos.y + NODE_R + 5}px`
      }
    }
    ...
    for (const rel of rels) {
      if (rel.sourceId !== itemId && rel.targetId !== itemId) continue
      const g = svgG.querySelector(`[data-rel-id="${rel.id}"]`)
      if (!g) continue
      const sp = rel.sourceId === itemId ? pos : nodePositions[rel.sourceId]
      const tp = rel.targetId === itemId ? pos : nodePositions[rel.targetId]
      if (!sp || !tp) continue
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const nodeEl = t.closest('[data-item-id]')
    if (nodeEl) {
      const itemId = nodeEl.getAttribute('data-item-id')!
      if (mode === 'connect') {
        const pos = nodePositions[itemId]; if (!pos) return
        setConnecting({ fromId: itemId, x: pos.x, y: pos.y })
      } else {
        const pos = nodePositions[itemId]; if (!pos) return
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    if (dragging) {
      ...
      applyDragToDOM(dragging.itemId)
      return
    }
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    if (dragging) {
      ...
      setNodePositions((p) => ({ ...p, [dragging.itemId]: { ...dragPosRef.current } }))
      setDragging(null)
      return
    }
    if (connecting) {
      ...
      const nodeEl = t.closest('[data-item-id]')
      if (nodeEl && graph) {
        const tid = nodeEl.getAttribute('data-item-id')!
        if (tid !== connecting.fromId)
          setCreatingRelation({ src: connecting.fromId, tgt: tid })
      }
      setConnecting(null)
    }
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (!itemId || !graph) return
    const pos = toCanvas(e.clientX, e.clientY)
    if (!graph.itemIds.includes(itemId)) {
      updateSynthesisGraph(graph.id, { itemIds: Array.from(new Set([...graph.itemIds, itemId])) })
      setNodePositions((p) => ({ ...p, [itemId]: { x: Math.round(pos.x), y: Math.round(pos.y) } }))
    }
  }, [graph, toCanvas, updateSynthesisGraph])

  const addToGraph = useCallback((itemId: string) => {
    if (!graph) return
    if (!graph.itemIds.includes(itemId)) {
      updateSynthesisGraph(graph.id, { itemIds: Array.from(new Set([...graph.itemIds, itemId])) })
      setNodePositions((p) => ({ ...p, [itemId]: { x: 380 + Math.round((Math.random() - 0.5) * 80), y: 300 + Math.round((Math.random() - 0.5) * 80) } }))
    }
    if (rightView === 'list') setRightView('canvas')
  }, [graph, rightView, updateSynthesisGraph])

  const removeFromGraph = (itemId: string) => {
    if (!graph) return
    const ids = graph.itemIds.filter((id) => id !== itemId)
    const rels = graph.relations.filter((r) => r.sourceId !== itemId && r.targetId !== itemId)
    updateSynthesisGraph(graph.id, { itemIds: ids, relations: rels })
    setNodePositions((p) => { const n = { ...p }; delete n[itemId]; return n })
  }

  const openGraph = (id: string) => { setCurrentSynthesisGraph(id); setRightView('canvas') }
  const handleCreateGraph = () => setShowGraphEditor({ isNew: true })
  const handleEditGraph = () => { if (graph) setShowGraphEditor({ isNew: false }) }

  const relations = graph?.relations || []
  const graphItems = useMemo(() => graphItemIds.map((id) => worldItems.find((i) => i.id === id)!).filter(Boolean), [graphItemIds, worldItems])

  const handleSaveRelation = (rel: SynthesisGraph['relations'][number]) => {
    if (rel.id) {
      setCurrentSynthesisGraph(graph?.id || null)
      updateSynthesisGraph(graph!.id, {
        relations: graph!.relations.map((r) =>
          r.id === rel.id ? { ...r, type: rel.type, direction: rel.direction, text: rel.text || undefined } : r
        ),
      })
    } else if (creatingRelation && graph) {
      const newRel = {
        id: crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).slice(2),
        sourceId: creatingRelation.src,
        targetId: creatingRelation.tgt,
        type: rel.type,
        direction: rel.direction,
        text: rel.text || undefined,
      }
      updateSynthesisGraph(graph.id, {
        relations: [...graph.relations, newRel],
      })
    }
    setEditingRelation(null); setCreatingRelation(null)
  }

  const openDetail = useCallback((item: Item) => setDetailItem(item), [])

  // ═══ 左侧列表项 ═══
  const listItem = (item: Item) => {
    const pc = primaryColor(item)
    const inGraph = graph && graphItemIds.includes(item.id)
    return (
      <div key={item.id} draggable
        onDragStart={(e) => { e.dataTransfer.setData('text/item-id', item.id); (e.target as HTMLElement).style.opacity = '0.5' }}
        onDragEnd={(e) => { (e.target as HTMLElement).style.opacity = '1' }}
        onClick={() => { if (inGraph) removeFromGraph(item.id); else addToGraph(item.id) }}
        ...
        {item.icon ? (
          ...
          <div style={{ width: '40px', height: '40px', borderRadius: '8px', background: pc, flexShrink: 0,
            border: `2px solid ${darken(pc, 0.35)}` }} />
        )}
        <span style={{ maxWidth: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 600, lineHeight: 1.2, textAlign: 'center', fontSize: '13px' }}>{item.name}</span>
      </div>
    )
  }
    ...
    <div style={{ display: 'flex', height: '100%', position: 'relative' }}>
      ...
      <div style={{ flex: '0 0 38%', minWidth: 0, display: 'flex', flexDirection: 'column', padding: '24px 22px', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
          <h2 style={{ fontSize: '18px', color: GOLD, margin: 0, letterSpacing: '2px' }}>合成图谱</h2>
        ...
        <input className="input" placeholder="搜索物品..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ marginBottom: '10px', fontSize: '13px', padding: '8px 12px' }} />
        <div style={{ fontSize: '11px', color: 'var(--color-text-muted)', marginBottom: '8px' }}>{leftItems.length} 件物品 · {graph ? `${graphItemIds.length} 在图谱中` : '未选择图谱'}</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
          {leftItems.length === 0 ? <div style={{ textAlign: 'center', padding: '30px', color: 'var(--color-text-muted)', fontSize: '13px' }}>无匹配物品</div> : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', alignContent: 'start', paddingRight: '4px' }}>{leftItems.map(listItem)}</div>}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: '24px 22px', background: 'rgba(245,230,200,0.15)' }}>
        {rightView === 'list' || !graph ? (
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <h3 style={{ fontSize: '16px', color: GOLD, marginBottom: '14px', letterSpacing: '1px' }}>合成关系图谱</h3>
            {worldGraphs.length === 0 ? (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px' }}>
                <SynthEmptyIcon />
                <p style={{ color: 'var(--color-text-muted)', fontSize: '13px' }}>尚未创建合成图谱</p>
              </div>
            ) : (
              <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
                {worldGraphs.map((g) => (
                  <div key={g.id} onClick={() => openGraph(g.id)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderRadius: '10px', cursor: 'pointer', marginBottom: '8px', border: `1.5px solid ${g.id === currentGraphId ? GOLD : 'var(--color-page-shadow)'}`, background: g.id === currentGraphId ? 'rgba(184,134,11,0.1)' : 'rgba(245,230,200,0.3)', transition: 'all 0.2s ease' }}
                    ...
                    <div style={{ flex: 1 }}><div style={{ fontSize: '15px', fontWeight: 600, color: 'var(--color-text)' }}>{g.name}</div><div style={{ fontSize: '11px', color: 'var(--color-text-muted)', marginTop: '2px' }}>{g.itemIds.length} 物品 · {g.relations.length} 关系</div></div>
            ...
            <div style={{ borderTop: '1px solid var(--color-page-shadow)', paddingTop: '12px', marginTop: '8px' }}>
              <div onClick={handleCreateGraph} style={{ padding: '12px', borderRadius: '8px', cursor: 'pointer', textAlign: 'center', border: `1.5px dashed ${GOLD}55`, color: GOLD, fontSize: '14px', fontWeight: 600 }}
                onMouseEnter={(ev) => { ev.currentTarget.style.background = 'rgba(184,134,11,0.08)'; ev.currentTarget.style.borderColor = GOLD_LIGHT }} onMouseLeave={(ev) => { ev.currentTarget.style.background = 'transparent'; ev.currentTarget.style.borderColor = `${GOLD}55` }}>+ 新建合成图谱</div>
            </div>
          </div>
        ) : (
          /* ── 画布视图 ── */
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px', flexShrink: 0 }}>
              ...
              <span style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>{graphItemIds.length} 物品 · {relations.length} 关系</span>
            </div>
            ...
            <div ref={canvasRef} style={{ flex: 1, position: 'relative', overflow: 'hidden', borderRadius: '8px', border: '3px solid var(--color-page-shadow)', background: '#e0ceaa', cursor: dragging ? 'grabbing' : panning ? 'grabbing' : mode === 'connect' ? 'crosshair' : 'grab' }}
              ...
              onMouseLeave={() => { if (dragging) { setNodePositions((p) => ({ ...p, [dragging.itemId]: { ...dragPosRef.current } })) }; setDragging(null); setPanning(null); if (!creatingRelation) setConnecting(null) }}
              ...
              <div style={{ position: 'absolute', transform: `translate(${offX}px,${offY}px) scale(${scale})`, transformOrigin: '0 0', width: 0, height: 0, zIndex: 1 }}>
                <svg style={{ position: 'absolute', inset: 0, overflow: 'visible', pointerEvents: 'none' }} width={1} height={1}>
                  <defs>
                    <marker id="sgCanvasArrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><polygon points="0,0 8,3 0,6" fill={GOLD} /></marker>
                    <marker id="sgCanvasArrowHover" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto"><polygon points="0,0 8,3 0,6" fill={GOLD_LIGHT} /></marker>
                  </defs>
                  <g ref={svgGRef} pointerEvents="auto">
                    {relations.map((rel) => {
                      const sp = dragging && rel.sourceId === dragging.itemId ? dragPosRef.current : nodePositions[rel.sourceId]
                      const tp = dragging && rel.targetId === dragging.itemId ? dragPosRef.current : nodePositions[rel.targetId]
                      ...
                      const me = showArrow ? (isHover ? 'url(#sgCanvasArrowHover)' : 'url(#sgCanvasArrow)') : undefined
                      const ms = rel.direction === 'twoWay' ? (isHover ? 'url(#sgCanvasArrowHover)' : 'url(#sgCanvasArrow)') : undefined
                      return (
                        <g key={rel.id} data-rel-id={rel.id}>
                          ...
                          <path d={d} fill="none" stroke={GOLD} strokeWidth={isHover ? 2.5 : 1.8} opacity={isHover ? 1 : 0.65}
                            markerStart={ms} markerEnd={me} style={{ transition: dragging ? 'none' : 'all 0.15s ease', pointerEvents: 'none' }} />
                          {rel.text && isHover && <text x={midX} y={midY - 10} textAnchor="middle" fill={GOLD} fontSize="11" fontWeight={600} style={{ pointerEvents: 'none' }}>{rel.text}</text>}
                        </g>
                      )
                    })}
                    {connecting && (
                      <line x1={nodePositions[connecting.fromId]?.x || 0} y1={nodePositions[connecting.fromId]?.y || 0} x2={connecting.x} y2={connecting.y}
                        stroke={GOLD_LIGHT} strokeWidth="2" strokeDasharray="6 4" opacity="0.7" markerEnd="url(#sgCanvasArrow)" pointerEvents="none" />
                    )}
                  </g>
                </svg>
                ...
                {graphItems.map((item) => {
                  if (!item) return null
                  const isDrag = dragging?.itemId === item.id
                  const pos = isDrag ? dragPosRef.current : nodePositions[item.id]; if (!pos) return null
                  const pc = primaryColor(item); const r = NODE_R
                  const isSource = connecting?.fromId === item.id
                  return (
                    <div key={item.id}>
                      <div data-item-id={item.id} style={{
                        ...
                        background: item.icon ? `#f5e6c8` : pc,
                        ...
                        cursor: mode === 'connect' ? (isSource ? 'pointer' : 'crosshair') : 'grab',
                        transition: isDrag ? 'none' : 'box-shadow 0.2s ease, border-color 0.2s ease',
                        zIndex: isDrag ? 10 : 2,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        overflow: 'hidden',
                      }}
                        onMouseEnter={() => { if (!dragging) setHoveredNodeId(item.id) }} onMouseLeave={() => { if (hoveredNodeId === item.id) setHoveredNodeId(null) }}
                        onDoubleClick={() => openDetail(item)} title="双击查看详情 · 从左侧面板移出图谱">
                        {hoveredNodeId === item.id && (
                          <div onClick={(ev) => { ev.stopPropagation(); ev.preventDefault(); removeFromGraph(item.id); setHoveredNodeId(null) }}
                        ...
                        {item.icon ? (
                          ...
                          <span style={{ fontSize: '16px', fontWeight: 700, color: '#f5e6c8', pointerEvents: 'none' }}>
                            {item.name.charAt(0)}
                          </span>
                        )}
                      </div>
                      <div style={{ position: 'absolute', left: pos.x, top: pos.y + r + 5, transform: 'translateX(-50%)', pointerEvents: 'none', zIndex: 1, textAlign: 'center', maxWidth: '90px' }}>
                        <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--color-text)', textShadow: '0 0 4px rgba(245,230,200,0.8)', lineHeight: 1.2 }}>{item.name}</span>
                      </div>
                    </div>
                  )
                })}
      ...
      {/* 弹窗 */}
      {showGraphEditor && <SynthGraphEditorModal isNew={showGraphEditor.isNew} graph={graph}
        onSave={(n, c) => { if (showGraphEditor.isNew) { addSynthesisGraph(n, c); setRightView('canvas') } else if (graph) updateSynthesisGraph(graph.id, { name: n, color: c }); setShowGraphEditor(null) }}
        onClose={() => setShowGraphEditor(null)} />}
      {deleteConfirm && <DeleteSynthGraphModal graphName={worldGraphs.find((g) => g.id === deleteConfirm)?.name || ''}
        onConfirm={() => { deleteSynthesisGraph(deleteConfirm); setDeleteConfirm(null); setRightView('list') }} onClose={() => setDeleteConfirm(null)} />}
      {editingRelation && <SynthRelationEditorModal relation={editingRelation}
        sourceName={worldItems.find((i) => i.id === editingRelation.sourceId)?.name || '?'} targetName={worldItems.find((i) => i.id === editingRelation.targetId)?.name || '?'}
        onSave={handleSaveRelation} onDelete={() => {
          if (graph) updateSynthesisGraph(graph.id, { relations: graph.relations.filter((r) => r.id !== editingRelation.id) })
          setEditingRelation(null)
        }} onClose={() => setEditingRelation(null)} />}
      {creatingRelation && <SynthRelationEditorModal relation={{ id: '', sourceId: creatingRelation.src, targetId: creatingRelation.tgt, type: 'line', direction: 'oneWay' } as SynthesisGraph['relations'][number]}
        sourceName={worldItems.find((i) => i.id === creatingRelation.src)?.name || '?'} targetName={worldItems.find((i) => i.id === creatingRelation.tgt)?.name || '?'}
        onSave={handleSaveRelation} onClose={() => setCreatingRelation(null)} />}
      {detailItem && <ItemDetailModal item={detailItem}
        onEdit={(item) => { updateItem(item.id, { name: item.name, description: item.description, tags: item.tags, attributes: item.attributes }); setDetailItem(null) }}
...
function ItemDetailModal({ item, onEdit, onClose }: { item: Item; onEdit: (item: Item) => void; onClose: () => void }) {
  ...
  return createPortal(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '520px', width: '90%', maxHeight: '85vh', overflowY: 'auto', overflowX: 'hidden' }}>
    <h2>{editing ? '编辑物品' : '物品详情'}</h2>
    <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '14px' }}>{item.tags.map((t) => (<span key={t.id} style={{ padding: '3px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 500, color: t.color, background: `${t.color}14`, border: `1px solid ${t.color}44` }}>{t.name}</span>))}</div>
    {editing ? (
      <>
        <div className="form-group"><label>名称 *</label><input className="input" value={nm} onChange={(e) => setNm(e.target.value)} autoFocus /></div>
        <div className="form-group">
          <label>标签</label>
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginBottom: '8px' }}>
            {item.tags.map((t) => (
              <span key={t.id} onClick={() => { onEdit({ ...item, name: nm.trim(), description: desc, tags: item.tags.filter((tt) => tt.id !== t.id) }) }}
                style={{ padding: '3px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 500, color: t.color, background: `${t.color}14`, border: `1px solid ${t.color}44` }}>{t.name} ✕</span>
            ))}
          </div>
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
            {worldTags.filter((t) => !item.tags.some((et) => et.id === t.id)).map((t) => (
              <span key={t.id} onClick={() => { onEdit({ ...item, name: nm.trim(), description: desc, tags: [...item.tags, t] }) }}
                style={{ padding: '3px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 500, color: t.color, background: `${t.color}10`, border: `1px solid ${t.color}33` }}>+ {t.name}</span>
            ))}
          </div>
        </div>
      ...
      <>
        <h3 style={{ fontSize: '20px', fontWeight: 700, color: 'var(--color-text)', margin: '0 0 16px' }}>{item.name}</h3>
        <div style={{ fontSize: '14px', color: 'var(--color-text-light)', lineHeight: '1.8', whiteSpace: 'pre-wrap', marginBottom: '16px' }}>{item.description || '暂无描述'}</div>
        {item.attributes && Object.keys(item.attributes).filter((k) => k.trim()).length > 0 && (
          <div style={{ marginBottom: '16px' }}>
            <div style={{ fontSize: '13px', color: 'var(--color-text-muted)', marginBottom: '6px', fontWeight: 600 }}>属性</div>
            {Object.entries(item.attributes).filter(([k]) => k.trim()).map(([k, v]) => (
              <div key={k} style={{ display: 'flex', padding: '4px 0', fontSize: '13px' }}>
                <span style={{ color: GOLD, minWidth: '80px', fontWeight: 600 }}>{k}</span>
    ...
    <div className="form-actions"><button className="btn" onClick={onClose}>关闭</button>{editing ? <button className="btn btn-primary" disabled={!nm.trim()} onClick={() => { onEdit({ ...item, name: nm.trim(), description: desc }); setEditing(false) }}>保存</button> : <button className="btn btn-primary" onClick={() => setEditing(true)}>编辑</button>}</div>
  </div></div>, document.body)
}

function SynthGraphEditorModal({ isNew, graph, onSave, onClose }: { isNew: boolean; graph: SynthesisGraph | null; onSave: (n: string, c: string) => void; onClose: () => void }) {
  const [nm, setNm] = useState(graph?.name || '')
  const [cl, setCl] = useState(graph?.color || GOLD)
  return createPortal(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '420px' }}>
    <h2>{isNew ? '创建合成图谱' : '编辑图谱'}</h2>
    <div className="form-group"><label>图谱名称 *</label><input className="input" value={nm} onChange={(e) => setNm(e.target.value)} placeholder="如：药剂合成、武器锻造..." autoFocus onKeyDown={(e) => { if (e.key === 'Enter' && nm.trim()) onSave(nm.trim(), cl) }} /></div>
...
function DeleteSynthGraphModal({ graphName, onConfirm, onClose }: { graphName: string; onConfirm: () => void; onClose: () => void }) {
  ...
  return createPortal(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '440px' }}>
    ...
    <div style={{ marginBottom: '18px', fontSize: '14px', color: 'var(--color-text-light)', lineHeight: '1.7', textAlign: 'center' }}>此操作不可撤销。图谱中的<b>所有合成关系</b>也将被永久删除。</div>
...
function SynthRelationEditorModal({ relation, sourceName, targetName, onSave, onDelete, onClose }: {
  relation: SynthesisGraph['relations'][number]; sourceName: string; targetName: string; onSave: (r: SynthesisGraph['relations'][number]) => void; onDelete?: () => void; onClose: () => void
}) {
  const isNew = !relation.id; const [type, setType] = useState(relation.type || 'line'); const [dir, setDir] = useState(relation.direction || 'oneWay'); const [text, setText] = useState(relation.text || '')
  const save = () => onSave({ ...relation, type, direction: dir, text })
  return createPortal(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '460px' }}>
    ...
    <div className="form-group"><label>关系描述（仅悬浮时显示）</label><input className="input" value={text} onChange={(e) => setText(e.target.value)} placeholder="如：炼制、锻造、合成、分解..." autoFocus onKeyDown={(e) => { if (e.key === 'Enter') save() }} /></div>
...
function SynthEmptyIcon() {
  return <svg viewBox="0 0 80 80" width="80" height="80" fill="none">
    <circle cx="40" cy="40" r="30" fill="none" stroke={GOLD} strokeWidth="2" opacity="0.35" />
    <polygon points="40,16 54,30 54,50 40,64 26,50 26,30" fill="none" stroke={GOLD} strokeWidth="1.5" opacity="0.4" />
    <circle cx="40" cy="40" r="6" fill={`${GOLD}20`} stroke={GOLD} strokeWidth="1.3" opacity="0.5" />
    <line x1="40" y1="34" x2="40" y2="28" stroke={GOLD} strokeWidth="1.2" opacity="0.35" />
    <line x1="34" y1="40" x2="28" y2="40" stroke={GOLD} strokeWidth="1.2" opacity="0.35" />
    <line x1="46" y1="40" x2="52" y2="40" stroke={GOLD} strokeWidth="1.2" opacity="0.35" />
    <circle cx="28" cy="28" r="3" fill={`${GOLD}12`} stroke={GOLD} strokeWidth="0.8" opacity="0.4" />
    <line x1="35" y1="35" x2="31" y2="31" stroke={GOLD} strokeWidth="0.8" opacity="0.25" />
    <circle cx="52" cy="28" r="3" fill={`${GOLD}12`} stroke={GOLD} strokeWidth="0.8" opacity="0.4" />
    <line x1="45" y1="35" x2="49" y2="31" stroke={GOLD} strokeWidth="0.8" opacity="0.25" />
  </svg>
}
